// Shared HTTP + checkpoint machinery for the US fetch scripts.
...
export const SEC_UA =
  "harness-skills buffett-screening-us (local test; contact: local-test@example.com)";

// Yahoo's chart API is the frozen price source; it wants a browser UA.
...
export async function throttle(host, minMs = 260) {
...
export async function fetchText(url, { headers = {}, method = "GET", body, retries = 4, minMs = 260, timeoutMs = 60000, host } = {}) {
...
export async function fetchBuffer(url, opts = {}) {
  const { headers = {}, minMs = 260, timeoutMs = 60000, host } = opts;
  const h = host ?? new URL(url).host;
  await throttle(h, minMs);
  const res = await fetch(url, {
    headers: { "User-Agent": BROWSER_UA, ...headers },
...
export async function saveRaw(runId, key, { url, body, meta = {} }) {
  ...
  await fs.writeFile(
    tmp,
    JSON.stringify({ url, fetched_at: new Date().toISOString(), mapping_version: MAPPING_VERSION, meta, body }, null, 1),
  );
...
export function parseArgs(argv) {
  const args = { runId: new Date().toISOString().slice(0, 10), positionals: [], minMs: 260 };
